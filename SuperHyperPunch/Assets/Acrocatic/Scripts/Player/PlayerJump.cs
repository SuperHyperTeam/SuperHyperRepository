...
namespace Acrocatic {
	...
	public class PlayerJump : MonoBehaviour {
        ...
        [Tooltip("Enter the input name to use for jump.")]
		...
		void Update () {
			...
			if(!jump && Input.GetButtonDown(jumpInput) && jumps > 0 && !player.dashing && !player.sliding && !player.onLadder && (!player.crouching || (player.crouching && player.AllowedToStandUp()))) {
